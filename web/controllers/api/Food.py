...
def foodIndex():
    ...
    cat_list = FoodCat.query.filter_by( status = 1 ).order_by( FoodCat.weight.desc() ).all()  # 分类列表
    data_cat_list = []
    ...
    food_list = Food.query.filter_by( status = 1 )\
        .order_by( Food.total_count.desc(),Food.id.desc() ).limit(3).all()   # 按销量倒序排 ，id倒序排
...
def foodInfo():
    ...
    resp['data']['info'] = {
        ...
        "price":str( food_info.price ),                                    # 字符串才能 转换在json中
