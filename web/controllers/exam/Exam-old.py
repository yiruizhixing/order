...
def index():
    ...
    return ops_render( "exam/index.html", resp_data )      # 将resp_data作为参数，用json传递给模板页面
...
def index():
    ...
    resp_data['search_con'] = req                   #搜索框内容
    resp_data['status_mapping'] = app.config['STATUS_MAPPING']
    return ops_render( "exam/kaodian.html", resp_data )      # 将resp_data作为参数，用json传递给模板页面

# 新增编辑考点
@route_exam.route( "/kaodian_set",methods = ["GET","POST"] )
def set():
    default_pwd = "******"                                  # 默认密码
    if request.method == "GET":
        resp_data = {}
