...
var food_set_ops = {
    ...
    eventBind: function () {
        ...
        $(".wrap_food_set .save").click(function () {
            ...
            if (name.length < 1) {
                common_ops.alert("请输入符合规范的标题~~");
                return;
            }

            if (summary.length < 10) {
                common_ops.tip("请输入内容描述，并不能少于10个字符~~", price_target);
                return;
            }
            ...
            $.ajax({
                url: common_ops.buildUrl("/peixun/set"),
                type: 'POST',
                data: data,
                dataType: 'json',
                success: function (res) {
                    ...
                    if (res.code == 200) {
                        callback = function () {
                            window.location.href = common_ops.buildUrl("/peixun/index");
                        }
                    }
